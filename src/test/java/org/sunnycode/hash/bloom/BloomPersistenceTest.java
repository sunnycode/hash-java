...
import org.sunnycode.hash.bloom.BloomFilter;
import org.sunnycode.hash.bloom.FilterState;
...
public class BloomPersistenceTest {
    public void testBloomPersist() {
        ...
        try {
            FilterState badState = new FilterState("bogus hash",
                    state.getState(), state.getMaxSize(),
                    state.getBitSetLength(), state.getK(), true);
